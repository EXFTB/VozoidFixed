...
function library.createkeybind(default, parent, blacklist, flag, callback, offset)
    ...
    utility.connect(services.InputService.InputBegan, function(input)
        if not binding and not game:GetService("UserInputService"):GetFocusedTextBox() and (input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == key) or input.UserInputType == key then
            callback(key)
        end
    end)
...
function library:Watermark2(str)
    local size = utility.textlength(str, Drawing.Fonts.Plex, 13).X
